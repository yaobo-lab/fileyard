...
pub fn extract_text(bytes: &[u8], mime_type: &str) -> Result<String, ExtractError> {
    match mime_type {
        ...
        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" => extract_xlsx(bytes),
        ...
        "application/vnd.ms-excel" => extract_xls(bytes),
...
fn extract_docx(bytes: &[u8]) -> Result<String, ExtractError> {
    ...
    let mut document_xml = match archive.by_name("word/document.xml") {
        Ok(file) => file,
        Err(_) => {
            return Err(ExtractError::OfficeError(
...
fn extract_pptx(bytes: &[u8]) -> Result<String, ExtractError> {
    ...
    if text_parts.is_empty() {
        return Err(ExtractError::OfficeError(
            "No text content found in presentation".to_string(),
        ));
    }
