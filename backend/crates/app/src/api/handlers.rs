...
use app_entity::repositories::{CreateFileParams, CreateFolderParams, ListFilesFilter};
        ...
        .map_err(|_| StatusCode::BAD_REQUEST)?
    {
        ...
        if tenant.approval_workflow_enabled.unwrap_or(false) {
                    ...
                    .await
            {
                ...
                let core_tenant: app_core::models::Tenant = tenant.clone().into();
...
pub async fn list_files(
    ...
    use app_core::cache::{hash_path, keys, ttl};
...
pub async fn list_trash(
    ...
    let rows = state
        ...
        .list_trash(
            tenant_id,
            app_entity::repositories::ListTrashFilter {
                target_owner,
                target_department,
                is_admin,
            ...
            if let Err(e) = state.storage.delete(fstorage).await {
                ...
                if state.replication_config.enabled
                    && state.replication_config.mode
                        == app_core::replication::ReplicationMode::Mirror
                {
                    let replication_store = state.store.clone();
                    let storage_key = fstorage.clone();
                    let tenant = tenant_id;
                    tokio::spawn(async move {
                        if let Err(e) = app_core::replication::enqueue_delete(
                            &replication_store,
                            &storage_key,
                            tenant,
                        )
...
pub async fn lock_file(
    ...
    if should_force_audit_log(&compliance_mode, "file_lock") {
        let _ = state
            .store
            .audit()
            .log_activity(app_entity::repositories::NewAuditLog {
                id: Uuid::new_v4(),
                tenant_id,
...
pub async fn create_file_share(
    ...
    if let Some(recipient_id) = shared_with_user_id {
        ...
        let tenant: Option<app_core::models::Tenant> = state
            .store
            .tenants()
            .by_id(tenant_id)
            .await
            .ok()
            .flatten()
            .map(app_core::models::Tenant::from);
...
pub async fn download_shared_file(
    ...
    if !is_public {
        ...
        if let Some(token_str) = auth_header {
            // Validate the token
            match crate::auth::verify_token(token_str) {
                Ok(claims) => {
