...
use rmqtt::{
    ...
    types::{Retain, TimestampMillis, Topic, TopicFilter, TopicName},
...
#[derive(Clone)]
pub(super) struct Retainer {
    inner: Arc<RetainerInner>,
}

impl Retainer {
    #[inline]
    pub(super) async fn new(cfg: Arc<PluginConfig>, storage_db: StorageDB) -> Result<Retainer> {
    ...
    pub(super) async fn info(&self) -> serde_json::Value {
        let msg_max = self.max().await;
        let msg_count = self.count().await;
        let msg_queue_count = self.msg_queue_count.load(Ordering::Relaxed);
    ...
    fn serve(self, mut msg_rx: mpsc::Receiver<Msg>) -> Result<Retainer> {
        ...
        tokio::spawn(async move {
            log::debug!("{log_prefix} starting serve ...");
            ...
            while let Some(msg) = msg_rx.next().await {
                ...
                log::debug!("{log_prefix} insert message len: {}", merger_msgs.len());
            ...
            log::error!("{log_prefix}  failed because receiver is gone");
        });
...
impl RetainerInner {
    ...
    async fn _batch_store(&self, msgs: Vec<Msg>) -> Result<()> {
        let (max_retained_messages, max_payload_size, retained_message_ttl) = {
            ...
            (
                cfg.max_retained_messages as usize,
                *cfg.max_payload_size,
                cfg.retained_message_ttl,
            )
        };

        let mut count = 0;
        for (topic_name, retain, expiry_interval) in msgs {
            let store_topic_name =
                [RETAIN_MESSAGES_PREFIX, topic_name.as_bytes().as_ref()].concat();
            if retain.publish.payload.is_empty() {
                ...
                {
                    log::warn!(
                        "{log_prefix} remove from db error, remove(..), {e:?}, topic_name: {topic_name:?}"
                    );
                };
            } else {
                match self
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        log::warn!(
                            "{log_prefix} store to db error, check_constraints(..), {e:?}, message: {retain:?}"
                        );
                        continue;
                    }
                }
                ...
                {
                    log::warn!(
                        "{log_prefix} store to db error, insert(..), {e:?}, message: {smsg:?}"
                    );
                    continue;
                };
                if let Some(expiry_interval_millis) = expiry_interval_millis {
                    ...
                    {
                        log::warn!(
                            "{log_prefix} store to db error, expire(..), {e:?}, message: {smsg:?}"
                        );
                        continue;
                    }
                }
    ...
    async fn check_constraints(
        ...
        if retain.publish.payload.len() > max_payload_size {
            log::warn!(
                "{log_prefix} message payload exceeding limit, topic: {topic:?}, retain: {retain:?}"
            );
            return Ok(false);
        }
        ...
        if max_retained_messages > 0 && self.get_retain_count().await >= max_retained_messages {
            log::warn!(
                "{log_prefix} The retained message has exceeded the maximum limit of: {max_retained_messages}, topic: {topic:?}, retain: {retain:?}"
            );
            return Ok(false);
        }
    ...
    async fn storage_messages_max_add(&self, vals: isize) -> Result<()> {
        self.storage_db
            .counter_incr(RETAIN_MESSAGES_MAX, vals)
            .await?;
        Ok(())
    }
    ...
    async fn get_message(&self, topic_filter: &TopicFilter) -> Result<Vec<(TopicName, Retain)>> {
        ...
        let mut iter = match db
            .scan([RETAIN_MESSAGES_PREFIX, topic_filter_pattern.as_bytes()].concat())
            .await
        {
            Err(e) => {
        ...
        while let Some(key) = iter.next().await {
            match key {
                Ok(key) => {
                    ...
                    if topic.matches_str(&String::from_utf8_lossy(
                        &key[RETAIN_MESSAGES_PREFIX.len()..],
                    )) {
...
impl RetainStorage for Retainer {
    ...
    async fn set(
        ...
        match res {
            ...
            Ok(Err(e)) => {
                log::error!("{log_prefix} Retainer set error, {e:?}");
                Err(anyhow!(e.to_string()))
            }
            Err(e) => {
                log::warn!("{log_prefix} Retainer store timeout, {e:?}");
                Err(anyhow!(e.to_string()))
            }
        }
    }
...
impl<T> ValueCached<T> {
        ...
        F: Future<Output = Result<T>> + Send,
    {
        ...
        ValueRef {
            val_guard: self.inner.read().await,
            cached,
            updating,
        }
    }
