...
pub(super) async fn subscribe(
    ...
    let tx = entry
        .tx()
        .ok_or_else(|| anyhow!("session message TX is not exist!"))?;
    let qos = qos.less_value(s.listen_cfg().max_qos_allowed);
    let subs = topics
        .iter()
        .map(|t| Subscribe::from_v3(t, qos, false, limit_subs))
    ...
    Ok(futures::future::join_all(reply_rxs)
        .await
        .into_iter()
        .collect())
}
...
#[inline]
pub(super) async fn unsubscribe(scx: &ServerContext, params: UnsubscribeParams) -> Result<()> {
    ...
    let tx = entry
        .tx()
        .ok_or_else(|| anyhow!("session message TX is not exist!"))?;
    let unsub = Unsubscribe::from(&topic_filter, false, limit_subs)?;
    let (reply_tx, reply_rx) = oneshot::channel();
    tx.unbounded_send(MqttMessage::Unsubscribe(unsub, reply_tx))
