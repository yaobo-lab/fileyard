...
use crate::{
    entities::{sso_attribute_mappings, tenants, users},
    DataResult,
};
...
impl<'a> SsoRepository<'a> {
    ...
    pub async fn mappings(
        ...
        protocol: &str,
        provider_id: Uuid,
    ) -> DataResult<Vec<sso_attribute_mappings::Model>> {
        Ok(sso_attribute_mappings::Entity::find()
            ...
            .order_by_asc(sso_attribute_mappings::Column::CreatedAt)
            .all(self.db)
            .await?)
    }

    pub async fn enabled_mappings(
        &self,
        protocol: &str,
        provider_id: Uuid,
    ) -> DataResult<Vec<sso_attribute_mappings::Model>> {
        Ok(sso_attribute_mappings::Entity::find()
            .filter(sso_attribute_mappings::Column::Protocol.eq(protocol))
            .filter(sso_attribute_mappings::Column::ProviderId.eq(provider_id))
            .filter(sso_attribute_mappings::Column::Enabled.eq(true))
            .order_by_desc(sso_attribute_mappings::Column::Priority)
            .order_by_asc(sso_attribute_mappings::Column::CreatedAt)
            .all(self.db)
            .await?)
    ...
    pub async fn identity_user_id(
        ...
        Ok(self
            .db
            .query_one(self.stmt(sql, vec![provider_id.into(), subject.into()]))
            .await?
            .map(|r| r.try_get("", "user_id"))
            .transpose()?)
    }

    pub async fn active_user(&self, id: Uuid) -> DataResult<Option<users::Model>> {
        Ok(users::Entity::find_by_id(id)
            .filter(users::Column::Status.eq("active"))
            .one(self.db)
            .await?)
    }
    pub async fn active_user_by_email(
        &self,
        tenant_id: Uuid,
        email: &str,
    ...
    pub async fn create_user(
        ...
        Ok(users::ActiveModel {
            ...
            custom_role_id: Set(custom_role_id),
            ...
            created_at: Set(now),
            updated_at: Set(now),
            identity_provider: Set(provider.into()),
        }
        .insert(self.db)
        .await?)
    }

    pub async fn set_hybrid(&self, id: Uuid) -> DataResult<()> {
        let Some(user) = users::Entity::find_by_id(id).one(self.db).await? else {
            return Ok(());
        };
        let mut active: users::ActiveModel = user.into();
        active.identity_provider = Set("hybrid".into());
    ...
    pub async fn touch_user(&self, id: Uuid) -> DataResult<()> {
        ...
        let mut active: users::ActiveModel = user.into();
        active.last_active_at = Set(Some(chrono::Utc::now().into()));
        active.update(self.db).await?;
        Ok(())
    }
    ...
    pub async fn link_identity(
        ...
        let (sql, values) = if protocol == "oidc" {
            ("INSERT INTO user_oidc_identities (user_id,provider_id,oidc_subject,oidc_issuer,oidc_email,oidc_name) VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (provider_id,oidc_subject) DO UPDATE SET oidc_email=EXCLUDED.oidc_email,oidc_name=EXCLUDED.oidc_name,updated_at=NOW()",vec![user_id.into(),provider_id.into(),subject.into(),issuer.into(),email.into(),name.into()])
        } else if protocol == "saml" {
            ("INSERT INTO user_saml_identities (user_id,provider_id,saml_name_id,saml_email,saml_name) VALUES ($1,$2,$3,$4,$5) ON CONFLICT (provider_id,saml_name_id) DO UPDATE SET saml_email=EXCLUDED.saml_email,saml_name=EXCLUDED.saml_name,updated_at=NOW()",vec![user_id.into(),provider_id.into(),subject.into(),email.into(),name.into()])
        } else {
            return Ok(());
        };
    ...
    pub async fn mapping(
        ...
        tenant_id: Uuid,
        id: Uuid,
    ) -> DataResult<Option<sso_attribute_mappings::Model>> {
        Ok(sso_attribute_mappings::Entity::find_by_id(id)
            .filter(sso_attribute_mappings::Column::TenantId.eq(tenant_id))
            .one(self.db)
            .await?)
    }
    ...
    pub async fn create_mapping(
        &self,
        input: NewSsoMapping,
    ) -> DataResult<sso_attribute_mappings::Model> {
        let now = chrono::Utc::now().into();
        Ok(sso_attribute_mappings::ActiveModel {
            ...
            attribute_name: Set(input.attribute_name),
            attribute_value: Set(input.attribute_value),
            match_type: Set(input.match_type),
            target_role: Set(input.target_role),
            target_custom_role_id: Set(input.target_custom_role_id),
            target_department_id: Set(input.target_department_id),
    ...
    pub async fn update_mapping(
        &self,
        existing: sso_attribute_mappings::Model,
        input: SsoMappingPatch,
    ) -> DataResult<sso_attribute_mappings::Model> {
        let mut model: sso_attribute_mappings::ActiveModel = existing.into();
        model.attribute_name = Set(input.attribute_name);
        model.attribute_value = Set(input.attribute_value);
        model.match_type = Set(input.match_type);
        model.target_role = Set(input.target_role);
        model.target_custom_role_id = Set(input.target_custom_role_id);
        model.target_department_id = Set(input.target_department_id);
    ...
    pub async fn delete_mapping(&self, tenant_id: Uuid, id: Uuid) -> DataResult<bool> {
        Ok(sso_attribute_mappings::Entity::delete_many()
            .filter(sso_attribute_mappings::Column::Id.eq(id))
            .filter(sso_attribute_mappings::Column::TenantId.eq(tenant_id))
            .exec(self.db)
            .await?
            .rows_affected
