...
use crate::{
    entities::{extension_installations, extension_permissions},
    DataResult,
};
...
pub struct ExtensionPermissionRepository<'a> {
...
impl<'a> ExtensionPermissionRepository<'a> {
    ...
    pub async fn installation(
        ...
        extension_id: Uuid,
        tenant_id: Uuid,
    ) -> DataResult<Option<ExtensionAccess>> {
    ...
    pub async fn has_for_tenant(
        &self,
        extension_id: Uuid,
        tenant_id: Uuid,
        permission: &str,
    ) -> DataResult<bool> {
        let Some(installation) = self.installation(extension_id, tenant_id).await? else {
            return Ok(false);
        };
    ...
    pub async fn grant(&self, installation_id: Uuid, permissions: &[String]) -> DataResult<()> {
        for permission in permissions {
            if !self.has(installation_id, permission).await? {
                extension_permissions::ActiveModel {
