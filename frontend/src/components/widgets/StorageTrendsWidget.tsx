    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 h-full">
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900 dark:text-white">{t('storageTrends')}</h3>
            ...
            {isLoading ? (
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center p-3 bg-primary-50 dark:bg-primary-900/20 rounded-lg">
                        ...
                        <div>
                            <p className="text-xs text-primary-600 dark:text-primary-400 font-medium">{t('currentUsage')}</p>
                    ...
                    <div>
                        <p className="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">
                            {t('lastDays', { days: period === '7d' ? 7 : period === '30d' ? 30 : 90 })}
                        </p>
                        <div className="flex items-end justify-between h-24 gap-2">
                            {trendData.map((item, index) => (
                                <div key={index} className="flex-1 flex flex-col items-center">
                                    ...
                                    ></div>
                                    <span className="text-xs text-gray-400 mt-1">{t('weekNum', { num: index + 1 })}</span>
                                </div>
                            ))}
                        </div>
                    </div>
